...
```
    ...
    1
    0
    <torch.cuda.device object at 0x000001A5956AD6D0>
...
![png](readme-ru_files/readme-ru_9_0.png)
...
![png](readme-ru_files/readme-ru_11_0.png)
...
class Perceptron(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            ...
            # Преобразует входную прямоугольную картинку в длинный одномерный тензор, не затрагивая ось батчей
            nn.Flatten(),
...
![svg](readme-ru_files/readme-ru_16_0.svg)
...
```
    ...
    Cell In[11], line 21, in model_train()
         18 for epoch in pbar:
         19     train_loss = 0.0  # Мониторим ошибку
    ---> 21     for images, labels in train_loader:
         22         images, labels = images.to(device), labels.to(device)
         24         # Очистка старых градиентов. Можно работать и с накоплением градиентов, но этот трюк не для данной (стандартной) ситуации
    

    File c:\Program Files\Python312\Lib\site-packages\torch\utils\data\dataloader.py:741, in _BaseDataLoaderIter.__next__(self)
    ...
    File c:\Program Files\Python312\Lib\site-packages\torch\utils\data\dataloader.py:801, in _SingleProcessDataLoaderIter._next_data(self)
    ...
    File c:\Program Files\Python312\Lib\site-packages\torch\utils\data\_utils\fetch.py:54, in _MapDatasetFetcher.fetch(self, possibly_batched_index)
...
![png](readme-ru_files/readme-ru_25_0.png)
...
Локально связный слой не входит в стандартную поставку PyTorch, но, к счастью, в людях иногда просыпается тяга к экспериментам, так что реализацию локально связного слоя найти всё-таки можно.
...
![png](readme-ru_files/readme-ru_46_0.png)
...
![png](readme-ru_files/readme-ru_59_0.png)
